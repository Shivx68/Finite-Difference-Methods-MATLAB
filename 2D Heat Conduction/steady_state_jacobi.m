% Function to solve 2D Steady state heat conduction equation using Jacobi iterative
% solver

function steady_state_jacobi(L, n, tolerance)
    ...
    while error > tolerance
        ...
        for j = 2: (n-1)
            for i = 2: (n-1)
                % jacobi method- simultaneous displacements
                T(i,j) = (1/k)*((T_old(i-1,j)+T_old(i+1,j))/dx^2+...
                    (T_old(i,j-1)+T_old(i,j+1))/dy^2);
            end
        end
